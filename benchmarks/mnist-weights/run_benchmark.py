#!/usr/bin/env python3
"""MNIST-Weights benchmark runner for fanout.

LLM writes raw float weights for a tiny MLP on sklearn's 8x8 digits.

Usage:
    uv run --extra benchmarks python benchmarks/mnist-weights/run_benchmark.py
    uv run --extra benchmarks python benchmarks/mnist-weights/run_benchmark.py --tasks binary_0v1
"""
...
TASKS = {
    "classify_all": {
        "file": "tasks/classify_all.py",
        "function": "classify_all",
        "description": "10-class digit classification with raw MLP weights (64->16->10)",
        "benchmark": 0.50,
    },
    "binary_0v1": {
        "file": "tasks/binary_0v1.py",
        "function": "binary_0v1",
        "description": "Binary digit classification: 0 vs 1 (64->8->1)",
        "benchmark": 0.90,
    },
    "binary_3v8": {
        "file": "tasks/binary_3v8.py",
        "function": "binary_3v8",
        "description": "Binary digit classification: 3 vs 8 (harder pair, 64->8->1)",
        "benchmark": 0.80,
    },
    "top5_accuracy": {
        "file": "tasks/top5_accuracy.py",
        "function": "top5_accuracy",
        "description": "Top-5 accuracy on 10-class digits (64->16->10)",
        "benchmark": 0.85,
    },
}
...
def build_prompt(task_name: str, task_info: dict) -> str:
    ...
    return (
        ...
        f"Keep the same function name and signature. Return a dict with numpy weight arrays. "
        f"The eval uses sklearn's load_digits (8x8 grayscale, 64 features, normalized to [0,1]). "
        f"You may use numpy. Think about what patterns distinguish the digits."
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run MNIST-Weights benchmarks with fanout")
    parser.add_argument(
    ...
    console.print()
    table = Table(title="MNIST-Weights Benchmark Results")
