#!/usr/bin/env python3
"""CIFAR-10 WeightGen benchmark runner for fanout.

LLM writes Python code that generates weights for fixed CNN architectures on CIFAR-10.
Hint: Use Gabor filters, edge detectors, color templates, mathematical patterns.

Usage:
    uv run --extra benchmarks python benchmarks/cifar10-weightgen/run_benchmark.py
    uv run --extra benchmarks python benchmarks/cifar10-weightgen/run_benchmark.py --tasks generate_cnn_small
"""
...
TASKS = {
    "generate_cnn_small": {
        "file": "tasks/generate_cnn_small.py",
        "function": "generate_cnn_small",
        "description": "Generate weights for small CNN on CIFAR-10 (~2K params)",
        "benchmark": 0.20,
    },
    "generate_cnn_medium": {
        "file": "tasks/generate_cnn_medium.py",
        "function": "generate_cnn_medium",
        "description": "Generate weights for medium 3-conv CNN on CIFAR-10 (~5K params)",
        "benchmark": 0.20,
    },
    "generate_linear_only": {
        "file": "tasks/generate_linear_only.py",
        "function": "generate_linear_only",
        "description": "Generate weights for linear-only model on CIFAR-10",
        "benchmark": 0.15,
    },
}
...
def build_prompt(task_name: str, task_info: dict) -> str:
    ...
    return (
        ...
        f"Keep the same function name and signature. Return a dict of numpy weight arrays. "
        f"The weights are loaded into a PyTorch model and evaluated on CIFAR-10 (2000 test images).\n\n"
        f"Hint: You can use Gabor filters, edge detectors, color templates, or other "
        f"mathematical patterns to craft weights that extract useful features. "
        f"You may use numpy. Think about what visual patterns distinguish CIFAR-10 classes "
        f"(airplane, automobile, bird, cat, deer, dog, frog, horse, ship, truck)."
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run CIFAR-10 WeightGen benchmarks with fanout")
    parser.add_argument(
    ...
    parser.add_argument("--model-set", "-M", help="Named model set")
    ...
    parser.add_argument("--solution-format", default="code")
    parser.add_argument("-p", "--eval-concurrency", type=int, default=1)
    parser.add_argument("--eval-timeout", type=int, default=120, help="Timeout per eval in seconds (default: 120)")
    parser.add_argument("--verbose", "-v", action="store_true")
    parser.add_argument("--full", action="store_true")
    parser.add_argument("--mode", choices=["sample", "agent"], default="sample")
    parser.add_argument("--n-agents", type=int, default=3)
    parser.add_argument("--max-steps", type=int, default=10)
    parser.add_argument("--memory", action="store_true", default=False)
    parser.add_argument("--record", nargs="?", const=True, default=None, metavar="NAME")
    parser.add_argument("--summary-model", default="anthropic/claude-sonnet-4-5")
    ...
    console.print()
    table = Table(title="CIFAR-10 WeightGen Benchmark Results")
