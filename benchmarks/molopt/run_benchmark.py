#!/usr/bin/env python3
"""MolOpt benchmark runner for fanout.

Evolve SMILES strings to optimize molecular properties via RDKit.

Usage:
    uv run --extra benchmarks python benchmarks/molopt/run_benchmark.py
    uv run --extra benchmarks python benchmarks/molopt/run_benchmark.py --tasks maximize_qed
"""
...
def build_prompt(task_name: str, task_info: dict) -> str:
    ...
    return (
        ...
        f"Keep the same function name and signature. Return a list of valid SMILES strings. "
        f"You may use rdkit for validation, but the eval will parse SMILES independently."
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run MolOpt benchmarks with fanout")
    parser.add_argument(
    ...
    console.print()
    table = Table(title="MolOpt Benchmark Results")
